...
# Add stem to String class
class String
  include Stemmable
end

# TODO: Test which method of removing stopwords is most efficient
#       - subtracttion from array
#       - regex
#       - using a set and test each word
...
class Sanitize
  ...
  def clean
    words_array = @text.downcase.gsub(ONE_OR_TWO_WORDS_RE, '').gsub(NON_ALPHANUMERIC_AND_NON_DOT_RE, ' ').gsub(@stopwords.to_re, '').gsub(/\./, '').split
    # words_array.map{ |word| word.stem }
  end

  def remove_stopwords
    word_array = @text.split
    (word_array - Stopword.stopwords).join(" ")
  end
end
...
class Stopword
  ...
  def initialize
    @stopwords = %w{a able about above abroad according accordingly across actually adj after afterwards again against ago ahead
